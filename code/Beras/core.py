from abc import ABC, abstractmethod, abstractproperty  # # For abstract method support
            ...
            for v, g in zip(self.inputs, self.input_gradients()):
                ...
                for b in range(batch_size):
                    ...
                    try: 
                        ...
                        raise ValueError(f"Error occured trying to perform `g_b @ j[b]` with {g_b.shape} and {j[b].shape}:\n{e}")
                J_out += [j_new]
        return J_out
...
class GradientTape:
    ...
    def __init__(self):
        self.prevs = defaultdict(lambda: None)  ## Previous layers, as recorded by Diffable
    ...
    def gradient(self, target, sources) -> list:
        ...
        grads = defaultdict(lambda: None)   ## Grads to be recorded. Initialize to None
        ...
        cut_graph = [f"var{i}" for i, grad in enumerate(out_grads) if grad is None]
        if cut_graph: print(f"Warning: The following tensors are disconnected from the target graph: {cut_graph}")
